...
async def create_user(user: UserSchema, request: Request, response: Response):
    ...
    try:
        new_user = User(name=user.name, email=user.email, password=hash_password(user.password), phone_number=user.phone_number, designation=user.designation, user_type=user.user_type)
        db.add(new_user)
        await db.commit()
