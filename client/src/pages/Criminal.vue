...
async function handleSubmit() {
  ...
  const { image, ...metadata } = newCriminal.value;
  if (image)
    formData.append('image', image)
  formData.append('metadata', JSON.stringify(metadata));

  try {
    console.log(newCriminal.value)
        ...
        <div class="bg-white p-6 rounded-lg shadow">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            <div>
              ...
              <input v-model="newCriminal.criminalname" id="criminalname" type="text" required
            ...
            <div>
              ...
              <input v-model="newCriminal.location" id="location" type="text" required
                class="w-full p-2 border rounded">
            </div>
            <div>
              <label for="cameraid" class="block mb-1">Camera ID</label>
              <input v-model.number="newCriminal.cameraid" id="cameraid" type="text" required
                class="w-full p-2 border rounded">
            </div>
            <div>
            ...
            <div>
              ...
              <input @change="handleFileChange" id="image" type="file" accept="image/*" required
                class="w-full p-2 border rounded">
            </div>
            <button type="submit" class="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600">Add
              Criminal</button>
          </form>
        ...
        <div class="bg-white p-6 rounded-lg shadow">
          ...
          <div v-else-if="criminals && criminals.length > 0" class="space-y-4">
            <div v-for="criminal in criminals" :key="criminal.id" class="border p-4 rounded">
              <img :src="'http://localhost:8000/' + criminal.images" :alt="criminal.criminalname"
